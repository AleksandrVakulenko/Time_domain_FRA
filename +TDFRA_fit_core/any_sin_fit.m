...
switch Amp_type
    case "const"
        Amp_str = TDFRA_fit_core.func_constructor([], 'a');
        a3 = mean(Est_amp);
        Lower = [Lower a3/10];
        StartPoint = [StartPoint a3];
        Upper = [Upper a3*10];
    case "linear"
        Amp_str = TDFRA_fit_core.func_constructor([X_arr(1) X_arr(3)], 'a');
...
switch BG_type
    case "const"
        BG_str = TDFRA_fit_core.func_constructor([], 'c');
        c3 = mean(Est_bg);
        Lower = [Lower -inf];
        StartPoint = [StartPoint c3];
        Upper = [Upper inf];
    case "linear"
        BG_str = TDFRA_fit_core.func_constructor([X_arr(1) X_arr(3)], 'c');
...
switch Phi_type
    ...
    case "linear"
        Phi_str = TDFRA_fit_core.func_constructor([X_arr(1) X_arr(3)], 'p');
...
function [Str, Lower, StartPoint, Upper] = ...
    ...
    switch Phi_type
        case "const"
            Str = TDFRA_fit_core.func_constructor([], Pref);
            p3 = mean(Est_v);
            Lower = [Lower -inf];
            StartPoint = [StartPoint p3];
            Upper = [Upper +inf];
        case "linear"
            Str = TDFRA_fit_core.func_constructor([X_arr(1) X_arr(3)], Pref);
