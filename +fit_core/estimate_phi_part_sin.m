...
if Fraction > 0.4 && Fraction <= 0.7
    Phase = fit_core.short_sig_phase_calc(Time, Signal, Period);
    Status = "OK";
    return
end
...
if Fraction > 0.7 && Fraction <= 1.4
    ...
    Phase1 = fit_core.short_sig_phase_calc(Time(1:N), Signal(1:N), Period);
    Phase2 = fit_core.short_sig_phase_calc(Time(N+1:end), Signal(N+1:end), Period);
